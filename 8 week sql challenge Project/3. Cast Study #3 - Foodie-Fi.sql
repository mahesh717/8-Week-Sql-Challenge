...
-- Insights : Foodie-Fi has 1000 customers.


-- 2. What is the monthly distribution of trial plan start_date values for our dataset - use the start of the month as the group by value ?
select monthname(start_date) as monthly_distribution, count(*) as month_trial 
from subs_plans
where plan_id = 0
group by monthly_distribution
order by month(start_date);

-- Insights : March has the biggest number of trial plan distribution.


-- 3. What plan start_date values occur after the year 2020 for our dataset? Show the breakdown by count of events for each plan_name ?
select count(*) as n_plans, plan_name
from subs_plans
where start_date >= '2020-01-01'
group by plan_name ;

-- Insight : The number of customers who churned the plan is the biggest one after the year 2020.


-- 4. What is the customer count and percentage of customers who have churned rounded to 1 decimal place?
create temporary table n_customers as (
select count(distinct(customer_id)) as n_customers
from subs_plans
);

 create temporary table n_churns as (
select count(distinct(customer_id)) as n_churns
from subs_plans
...
n_customers,
n_churns,
round((n_churns /n_customers )*100,1) as churn_percent
from n_customers, n_churns;

-- Insights : It is 30.7% of customers who have churned the plans.
...
from subs_plans)
select 
count(prev_plan) as n_churn,
round(count(*)*100/(select count(distinct customer_id) from subs_plans),0)as perc_churn
from churn_cte
where plan_id = 4 and prev_plan = 0 ;

-- Insights: There are 92 customers who have churned straight after their initial free trial, which 9% of the customer base.



-- 6. What is the number and percentage of customer plans after their initial free trial ?
select * from subs_plans ; 

with cte_nxt_plan as (
select *,
...
round(count(*)*100/(select count(distinct customer_id) from subs_plans),1) as percent_nxt_plan
from cte_nxt_plan
where nxt_plan is not null and plan_id = 0
group by nxt_plan
order by nxt_plan;
...
-- 7. How many customers have upgraded to an annual in 2020 ?
select count(customer_id) as n_customers
from subs_plans
where plan_id = 3 and start_date <= '2020-12-31' ;

-- Insights : There’s 195 customers who have upgraded to annual in 2020.
